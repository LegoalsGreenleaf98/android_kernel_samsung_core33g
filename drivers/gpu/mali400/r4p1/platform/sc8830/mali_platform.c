...
static const struct gpu_freq_info* get_next_freq(const struct gpu_freq_info* min_freq,
...
void mali_platform_utilization(struct mali_gpu_utilization_data *data)
{
	...
	/* if the loading ratio is greater then up_threshold, switch the clock to the maximum and
	 * plug in all GPU cores
	 */
